...
st.text("""Я хочу внимательнее посмотреть на таблицы с ГИС торги. 
...
st.text("""Результаты подтверждают мою гипотезу. 
...
Это также подтверждается статьями, в которых исследуется увеличение 
тяги к риску в определенное время года, связь риска с погодой. 
Это достаточно ценный вывод, который мжет помочь ппродавцам 
недвижимости поднять цену на объект. 
...
st.text("""Теперь посмотрим на зависимость 
...
st.text("""Так и получилось. На объекты внутри КАДа цена растет быстрее, 
...
st.write(r"""ВРИ - вид разрешенного использования участка. 
